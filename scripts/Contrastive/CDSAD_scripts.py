...
from src.models.CDSAD import CDSAD
...
def main(config_path):
    """
    Train a DSAD on the MURA dataset using a Contrsative pretraining.
    """
    # Load config file
    ...
    for seed_i, seed in enumerate(cfg.settings['seeds']):
        ...
        # make model
        clr_DSAD = CDSAD(net_CLR, net_DSAD, tau=cfg.settings['SimCLR']['tau'],
        ...
        # Train SimCLR
        clr_DSAD.train_contrastive(train_dataset_CLR, valid_dataset=None,
        ...
        clr_DSAD.evaluate_contrastive(valid_dataset_CLR, batch_size=cfg.settings['SimCLR']['batch_size'],
                                 n_job_dataloader=cfg.settings['SimCLR']['num_worker'],
                                 device=cfg.settings['device'],
                                 print_batch_progress=cfg.settings['print_batch_progress'],
                                 set='valid')

        clr_DSAD.evaluate_contrastive(test_dataset_CLR, batch_size=cfg.settings['SimCLR']['batch_size'],
                                 n_job_dataloader=cfg.settings['SimCLR']['num_worker'],
                                 device=cfg.settings['device'],
                                 print_batch_progress=cfg.settings['print_batch_progress'],
                                 set='test')
        ...
        # save repr net
        clr_DSAD.save_repr_net(OUTPUT_PATH + f'model/Contrastive_net_{seed_i+1}.pt')
